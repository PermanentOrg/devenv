...
if $DELETE_DATA; then
  ...
  aws s3 rm s3://permanent-local/$SQS_IDENT --recursive
fi
...
aws s3 cp --recursive s3://permanent-local/certs /tmp/certs
mv /tmp/devenv/certs/* /etc/ssl
...
rm -rf node_modules
cd /data/www/upload-service && cp package.json ~ && cp package-lock.json ~
cd ~ && npm install
rm package.json package-lock.json && mv node_modules /data/www/upload-service/
cd /data/www/upload-service && npm run build

echo "Configure PHP application"
cd /data/www/library
rm -rf vendor/
cd /data/www/library && php bin/composer.phar install --no-plugins
...
cd /data/www/back-end && php library/bin/composer.phar install --prefer-dist --working-dir=tools/phplint
cd /data/www/back-end && php library/bin/composer.phar install --prefer-dist --working-dir=tools/php-cs-fixer
